...
public class AirportTest {
    private static final List<Plane> planeList = Arrays.asList(
    ...
    public void testHasAtLeastOneTransportInMilitaryPlanes() {
        Assert.assertTrue(new Airport(planeList).getTransportMilitaryPlanes().stream()
    ...
    public void testGetPassengerPlaneWithMaxPassengerCapacity() {
        Assert.assertEquals(
                new PassengerPlane("Boeing-747", 980, 16100, 70500, 242),
                new Airport(planeList).getPassengerPlaneWithMaxPassengersCapacity()
        );
    }

    @Test
    public void testHasAtLeastOneBomberInMilitaryPlanes() {
        Assert.assertTrue(new Airport(planeList).getBomberMilitaryPlanes().stream()
                .anyMatch(militaryPlane -> militaryPlane.getType() == MilitaryType.BOMBER));
    }

    @Test
    public void testExperimentalPlanesHasClassificationLevelHigherThanUnclassified(){
        Assert.assertFalse(new Airport(planeList).getExperimentalPlanes().stream()
